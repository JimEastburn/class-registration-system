...
export function MasterCalendarGrid() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Check if changed
    if (
      pattern === eventData.rawConfig.day &&
      block === eventData.rawConfig.block
    ) {
      return;
    }
  ...
  const events = useMemo(() => {
    ...
    const mappedEvents: (CalendarUIEvent & { rawConfig: ScheduleConfig })[] =
      [];
    // Detect conflicts
    const conflictingClassIds = detectBatchConflicts(
      classes as unknown as Class[]
    );

    classes.forEach((cls) => {
      if (!cls.schedule_config || !['published', 'draft'].includes(cls.status))
        return;
      ...
      <DragOverlay>
        {activeDragEvent ? (
          <div className="w-[200px] opacity-80">
            <CalendarEventCard event={activeDragEvent} isMonthView={false} />
          </div>
        ) : null}
      </DragOverlay>
