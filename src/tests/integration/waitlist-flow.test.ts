...
describe('Integration Flow: Waitlist -> Promotion', () => {
  ...
  it('promotes waitlisted student when spot opens via refund', async () => {
    ...
    const enroll1 = await enrollStudent({
      classId: mockClassId,
      familyMemberId: 'student-1',
    });
    ...
    mockSupabase.auth.getUser.mockResolvedValueOnce({
      data: { user: { id: 'parent-2' } },
      error: null,
    });
    const enroll2 = await enrollStudent({
      classId: mockClassId,
      familyMemberId: 'student-2',
    });
    expect(enroll2.status).toBe('waitlisted');

    // Verify Waitlist State
    const student2EnrollmentId = enroll2.data!.id;
    const student2Enrollment = dbEnrollments.find(
      (e) => e.id === student2EnrollmentId
    );
    ...
    expect(
      dbEnrollments.find((e) => e.id === dbEnrollments[0].id)?.status
    ).toBe('cancelled');

    // Student 2 should be 'pending' (promoted) and waitlist_position null
    const promotedEnrollment = dbEnrollments.find(
      (e) => e.id === student2EnrollmentId
    );
