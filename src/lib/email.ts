...
export async function sendEnrollmentCancellation(data: CancellationEmailData) {
  if (!resend) {
    console.log('Email not configured - skipping enrollment cancellation email');
    return { success: false, error: 'Email not configured' };
  }
  ...
  try {
    const result = await resend.emails.send({
      from: `${APP_NAME} <${FROM_EMAIL}>`,
      to: data.parentEmail,
      subject: `Enrollment Cancelled: ${data.studentName} - ${data.className}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          ...
          <div class="container">
            <div class="card">
              <div class="header">
                ...
                <div class="alert-badge">Enrollment Cancelled</div>
              </div>
              
              <p>Hi ${data.parentName},</p>
              <p>We wanted to let you know that <strong>${data.studentName}</strong>'s enrollment in <strong>${data.className}</strong> has been cancelled by the teacher or an administrator.</p>
              
              <p>If you have already made a payment, please contact an admin to discuss a refund.</p>
            ...
            <div class="footer">
              <p>© ${new Date().getFullYear()} AAC Austin Arts & Academic Collaborative - Class Registration System. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
    ...
    console.error('Failed to send enrollment cancellation email:', error);
    return { success: false, error };
  }
}
