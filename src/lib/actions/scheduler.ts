...
> {
  try {
    ...
    if (
      !profile ||
      !['class_scheduler', 'super_admin'].includes(profile.role)
    ) {
    ...
    const conflictCount =
      conflictResult.success && conflictResult.data
        ? conflictResult.data.length
        : 0;
...
export async function schedulerUpdateClass(
  id: string,
    ...
    if (proposedState.location && proposedState.schedule_config) {
      ...
      if (roomClasses) {
        ...
        if (conflict) {
          return {
            success: false,
            error: `Room Conflict: ${conflict.name} uses ${conflict.location} at ${conflict.schedule_config?.day} ${conflict.schedule_config?.block}`,
          };
        }
      }
    }
    ...
    if (data.teacher_id && data.schedule_config) {
      ...
      if (teacherClasses) {
        ...
        if (conflict) {
          return {
            success: false,
            error: `Teacher Conflict: ${conflict.name}`,
          };
        }
      }
    }
    ...
    if (data.location && data.schedule_config) {
      ...
      if (roomClasses) {
        ...
        if (conflict) {
          return {
            success: false,
            error: `Room Conflict: ${conflict.name} is already using ${conflict.location}`,
          };
        }
      }
    }
