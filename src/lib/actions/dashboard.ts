      ...
      .map((e) => {
        ...
        return {
          ...
          teacherName: classData.profiles
            ...
            : 'TBD',
          familyMemberName: familyMemberMap.get(e.student_id) || 'Unknown',
        };
      });
...
export async function getTeacherDashboardData(): Promise<{
  ...
  try {
    ...
    const classesTodayList =
      classes?.filter((c) => {
        const config = c.schedule_config as ScheduleConfig | null;
        const classDay = c.day || config?.day;
        return (
          classDay?.toLowerCase() === todayDayOfWeek &&
          isSchedulableStatus(c.status)
        );
      }) || [];
    ...
    const todayClasses: TodayClass[] = classesTodayList.map((c) => ({
      ...
      name: c.name,
      block:
        c.block || (c.schedule_config as ScheduleConfig | null)?.block || 'TBA',
      enrolledCount:
        enrollmentCounts.find((e) => e.class_id === c.id)?.count || 0,
    ...
    if (classIds.length > 0) {
      ...
      recentEnrollments = (enrollmentData || []).map((e) => {
        const fm = e.family_members as unknown as {
          first_name: string;
          last_name: string;
        } | null;
