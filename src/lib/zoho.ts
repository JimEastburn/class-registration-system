...
const ZOHO_INVOICE_SUBJECT =
  process.env.ZOHO_INVOICE_SUBJECT || "AAC Fall '26 Registration";
const ZOHO_TERMS_AND_CONDITIONS =
  'Thank you for registering with Austin AAC. All fees are non-refundable unless the class is canceled by the organization.';
...
async function getAccessToken(): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get Zoho access token: ${data.error || response.statusText}`
    );
  }
...
export async function syncPaymentToZoho(paymentId: string) {
  try {
    ...
    const invoiceId = await createZohoInvoice(
      contactId,
      enrollment,
      payment,
      accessToken
    ...
    console.log(
      `Successfully synced payment ${paymentId} to Zoho Books (Invoice: ${invoiceId})`
    );
...
async function findZohoContact(
  email: string,
  token: string
): Promise<string | null> {
  const response = await fetch(
...
async function createZohoContact(
  ...
  const data = await response.json();
  if (data.code !== 0)
    throw new Error(`Zoho Contact creation failed: ${data.message}`);
...
async function createZohoInvoice(
  ...
  const data = await response.json();
  if (data.code !== 0)
    throw new Error(`Zoho Invoice creation failed: ${data.message}`);
...
async function recordZohoPayment(
  ...
  const data = await response.json();
  if (data.code !== 0)
    throw new Error(`Zoho Payment recording failed: ${data.message}`);
}
