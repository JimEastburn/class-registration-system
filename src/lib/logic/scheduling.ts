...
export function validateScheduleConfig(config: ScheduleConfig): {
  ...
  if (!VALID_DAYS.includes(config.day as ValidDay)) {
    return {
      valid: false,
      error: `Invalid day: ${config.day}. Classes can only be scheduled on Tuesday, Wednesday, or Thursday.`,
    };
  }
...
export function checkScheduleConflict(
  ...
  for (const existingClass of existingClasses) {
    ...
    if (
      newClassConfig.day === existingConfig.day &&
      newClassConfig.block === existingConfig.block
    ) {
