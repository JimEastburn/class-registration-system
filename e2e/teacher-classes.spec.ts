...
test.describe('Teacher Class Management', () => {
  ...
  test('Teacher can create and publish a class', async ({ page }) => {
    ...
    await page
      ...
      .fill('This is a test class created by E2E test.');
  ...
  test('Teacher sees validation errors when submitting empty form', async ({
    page,
  }) => {
    // Login
    const loginPage = new LoginPage(page);
    ...
    await expect(
      errorSummary.getByText('Name must be at least 3 characters')
    ).toBeVisible();
  });
});
