...
test.describe('Parent Enrollment Flow', () => {
  ...
  test('Parent can browse and enroll child in a class', async ({ page }) => {
    ...
    await page
      ...
      .fill('Test class for enrollment E2E');
    ...
    await expect(page)
      .toHaveURL(/\/teacher\/classes\/.+/, { timeout: 30000 })
      .catch(async () => {
        const errorAlert = page.locator('.text-destructive');
        if (await errorAlert.isVisible()) {
          throw new Error(
            `Class creation failed: ${await errorAlert.textContent()}`
          );
        }
    ...
    console.log(
      `[DEBUG] DB Classes for teacher ${teacherUser.userId}:`,
      dbClasses?.length,
      dbClasses
    );
    ...
    await expect(
      page.getByText(`${childFirstName} ${childLastName}`)
    ...
    const specificChildTrigger = page
      .locator('button')
      .filter({ hasText: /Select a child/ })
      .first();
    ...
    // Select the child
    await page
      .getByRole('option', { name: `${childFirstName} ${childLastName}` })
      .click();
    ...
    // Verify Success
    await expect(
      page.getByText(/Enrollment successful|Successfully enrolled/i)
